...
Summary:          PackYak automated build of package = chardet (4.0.0)
...
Summary:        %{summary}
Provides:         python3.8dist(chardet)
...
rm -f $RPM_BUILD_ROOT/usr/bin/chardetect
sed -i '/\/usr\/bin\/chardetect/d' ./INSTALLED_FILES
%check
...
%files -n  python3-bio-chardet -f INSTALLED_FILES
%defattr(-,root,root)

%changelog
* Sat Feb 13 2021 sagrudd <stephen@mnemosyne.co.uk>
- first build of [chardet] version [4.0.0] by PackYak v0.0.7
